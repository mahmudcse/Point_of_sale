...
<div style="width: 400px;float: left;">	<label>Select role: </label><?php echo form_dropdown('roleId',$role, $roleId,"onchange=\"this.form.submit();\"");?></div>
</div>
<div class="box">
			...
			<div id="search_result">			
				...
				<?php					$cnt = 0;
					if($searchData != null && count($searchData) > 0)
					{
						...
						{								$cnt++				?>							<li><input type="checkbox" id="function<?php echo $cnt;?>" <?php echo $value->assigned!=0?"checked":"";?> name="functions[]" value="<?php echo $value->componentId;?>"/> &nbsp;&nbsp;<label style="display: inline;" for="function<?php echo $cnt;?>"><?php echo $value->uniqueCode;?></label></li>				<?php 
						}
					}
				?>
